self.addEventListener('message', function(e) {
    ...
    function performAction(events, ms, attacker, defender) {
        ...
        attacker.abilities.forEach(ability => {
            if (["MH Swing", "OH Swing", "Auto Attack"].includes(ability.name))
                ability.currentCooldown = ability.currentCooldown - globals.config.timeStep*attacker.getHaste();
    ...
    if (globals.tankStats.stance == "Defensive Stance") playerAbilities.push(new Revenge("Revenge", 5000, 5, true, 273, 2.25))
    ...
    playerAbilities.push(new SunderArmor("Sunder Armor", 0, 15 - globals.tankStats.talents.impSA - (globals.tankStats.bonuses.threePieceBrotherhood ? 5 : 0), true, 260))
    ...
    for (let i in range(iterations)) {
        ...
        while(ms <= globals.config.simDuration*1000) {
            ...
            if(globals.config.shart && !shartApplied && ms >= globals.config.shartdelay) {
                Boss.auras.forEach(aura => {
                    if(aura.name == "Shattered Armor") aura.handleEvent(Boss, { "type": "damage", "ability": "Shattered Armor", "hit": "hit", "timestamp": ms}, events, globals.config);
                })
                ieaApplied = true;
            }
